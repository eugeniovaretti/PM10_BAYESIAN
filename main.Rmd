...
ggsave("output_plot/ts_pacf_random.svg", plot_ACF, width = 10, height = 10)
```
...
prova = calcola_coordinate(località)
```

```{r}
library(raster)
dist_lonlat = pointDistance(prova$coordinate_long_lat, prova$coordinate_long_lat, lonlat=T,
allpairs=T)/ 1000
```
