...
save_svg_plot(plot_PACF, name="ts_pacf_random", folder="output_plot/", type="ggplot", width = 10, height = 10)
```
...
```{r}
plot_coeff_old = plot_coefficienti_AR1(ob4$coefficienti, ob3$area_località, ob3$tipo_località)
...
```{r}
label = TRUE
ob6 = plot_clustering(partitions, coord, "VI" ,M, a, ts_read, state_params, label = label)
ob6
...
if(label)
{
  ...
  save_svg_plot(ob6$plot,name=nome, folder="output_plot/", type="ggplot",width = 15, height = 15)
}else if (!label)
{
  nome = paste("map_M_", M, "_a_",a,sep="")
  save_svg_plot(ob6$plot,name=nome, folder="output_plot/", type="ggplot",width = 10, height = 10)
}
